...
{
    private static readonly Dictionary<string, (string Namespace, string Url)> _controllerData = new()
    {
        {
    ...
    public async Task EnsureIngressController(string context, string controller)
    {
        ...
        if (!_controllerData.TryGetValue(controller, out var data))
        {
        ...
        var tempFile = fileSystem.Path.Combine(fileSystem.Path.GetTempPath(), $"ingress-{fileSystem.Path.GetRandomFileName()}.yaml");
